...
const TeacherLayout: React.FC = () => {
    ...
    const navItems = useMemo<NavItem[]>(() => [
        { to: '/teacher', label: 'Dashboard', icon: <LayoutDashboard size={18} />, match: (p) => p === '/teacher' },
        { to: '/teacher/classes', label: 'Kelas', icon: <Users size={18} />, match: (p) => p.startsWith('/teacher/classes') },
        { to: '/teacher/materials', label: 'Materi', icon: <BookOpen size={18} />, match: (p) => p.startsWith('/teacher/materials') },
        { to: '/teacher/curriculum', label: 'Kurikulum', icon: <FileStack size={18} />, match: (p) => p.startsWith('/teacher/curriculum') },
        { to: '/teacher/analytics', label: 'Analitik', icon: <BarChart3 size={18} />, match: (p) => p.startsWith('/teacher/analytics') },
    ], []);

    const bottomNav = useMemo(() => [
        { to: '/teacher', icon: <LayoutDashboard size={20} />, label: 'Home', match: (p: string) => p === '/teacher' },
        { to: '/teacher/classes', icon: <Users size={20} />, label: 'Kelas', match: (p: string) => p.startsWith('/teacher/classes') },
        { to: '/teacher/materials', icon: <BookOpen size={20} />, label: 'Materi', match: (p: string) => p.startsWith('/teacher/materials') },
        { to: '/teacher/curriculum', icon: <FileStack size={20} />, label: 'Kurikulum', match: (p: string) => p.startsWith('/teacher/curriculum') },
        { to: '/teacher/analytics', icon: <BarChart3 size={20} />, label: 'Analitik', match: (p: string) => p.startsWith('/teacher/analytics') },
    ], []);
        ...
        <div style={{ display: 'flex', minHeight: '100vh', background: '#fafafa' }}>
            <aside style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', padding: '0 0.5rem', marginBottom: '1.5rem' }}>
                    <img src="/logo-Photoroom-v2.webp" alt="Geo Education" style={{ width: 44, height: 44, objectFit: 'contain' }} />
                    <div>
                    ...
                    {navItems.map((item) => {
                        ...
                        return (
                            <Link key={item.to} to={item.to} style={{
                                ...
                                color: active ? '#2563eb' : 'var(--gray-600)',
                ...
                <div style={{ borderTop: '1px solid var(--gray-200)', paddingTop: '1rem', marginTop: '0.5rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', padding: '0.5rem' }}>
                        ...
                        <div style={{ minWidth: 0, flex: 1 }}>
                            <p style={{ fontSize: '0.8125rem', fontWeight: 500, color: 'var(--gray-900)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user?.name || 'Guru'}</p>
                            <p style={{ fontSize: '0.6875rem', color: 'var(--gray-400)' }}>Guru</p>
                        </div>
