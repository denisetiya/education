...
router.get('/map', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user.id;
        const progress = await prisma_1.default.progress.findMany({
            where: { userId },
...
router.get('/material/:materialId', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user.id;
...
router.post('/start', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user.id;
...
router.put('/update', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        const updated = await prisma_1.default.progress.upsert({
            where: {
                userId_materialId: { userId, materialId }
            },
            update: {
...
// Complete a material
router.post('/complete', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user.id;
...
router.get('/quiz-passed/:materialId', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user.id;
        const { materialId } = req.params;
        ...
        const quizProgress = await prisma_1.default.progress.findUnique({
            where: {
